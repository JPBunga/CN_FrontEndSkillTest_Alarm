function updateClock() {
  const now = new Date();
  const hours = now.getHours() > 12 ? (now.getHours() - 12).toString().padStart(2, '0') : now.getHours().toString();
...
// This sound clip plays when the alarm hits its time
var sound = new Audio("AlarmTone.mp3")
sound.loop = true;

// Selected by the visitor :

// getting components for setting the alarm
let arr = [];
let setHr = document.getElementById("alarmHrs");
let setMin = document.getElementById("alarmMins");
let setSec = document.getElementById("alarmSecs");
let setAMPM = document.getElementById("alarmAmpm");
